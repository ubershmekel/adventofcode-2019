...
function get_points(path_str) {
  // console.log(path_str)
  const points = new Set();
  let at = [0, 0];
  for (const move of path_str.split(',')) {
    const dir = move[0];
    let count = +move.substring(1);
...
for (const test of tests) {
  const paths = test.input.split('\n');
  const points = paths.map(get_points);
  //console.log(points[0].values().map(console.log))
  // for (const item of points[0]) {
  //   console.log(item)
  // }
  // console.log('---')
  // for (const item of points[1]) {
  //   console.log(item)
  // }
  // console.log([...points[0]].length);
  // console.log([...points[1]].length);

  const intersection = new Set([...points[0]].filter(x => points[1].has(x)));
  // console.log([...intersection].length)
  let minDistance = 999999;
  let minPoint = null;
  for (const item of intersection) {
    console.log(item)
    const xy = JSON.parse(item);
